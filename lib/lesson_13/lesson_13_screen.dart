...
//используется для позиционирования одного дочернего елемента
//с помощью координат
...
// Padding уменьшает размеры контейнера, margin сохраняет
//и делает отступы снаружи
...
// Відповідь: ConstrainedBox не застосовуються, тому що SizedBox задет
// жесткие constraints а дочерний елемент не может быть больше
// за constraints родителя
...
//OverflowBox для этого задания он не нужен потому что позволяет
//выходить за рамки родителя
...
/// Відповідь: зеленый накладывается на красный потому что
/// OverflowBox позволяет выходить за родительские границы,
/// не накладывается на синий потому что в Column рисуются сверху вниз
/// по порядку дочерние елементы
...
// Відповідь: потому что LimitedBox работает только если unbounded constraints
// в первом случае unbounded constraints а во втором уже
// жесткие ограничения от SizedBox
...
// Відповідь: потому что сам екран дал значения
// constraints высота максимум = экран , тоесть не бесконечное значение
// что уже не unbounded constraints
